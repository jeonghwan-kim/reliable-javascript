...
describe('Aop', function () {
  ...
  describe('Aop.around(fnName, advice, targetObj)', function () {
    ...
    it('마지막 어드바이스가 기존 어드바이스에 대해 실행되는 방식으로 체이닝할 수 있다', function () {
      ...
      Aop.around('targetFn', adviceFactory('inside'), targetObj);
      Aop.around('targetFn', adviceFactory('outside'), targetObj);
      targetObj.targetFn();
      expect(executionPoints).toEqual([
        'wrappingAdvice - start outside',
        'wrappingAdvice - start inside',
        'targetFn',
        'wrappingAdvice - end inside',
        'wrappingAdvice - end outside'
      ])
    });
